...
echo ================================================
echo Creolight patch sync (legacy fallback)
echo Recommended: run 直接安装到ProgramFiles.bat instead
echo ================================================
echo.
...
(
  ...
  echo "%%APP_DIR%%\python_venv\Scripts\python.exe" "%%PATCH_RUNTIME%%\run_patched_app.py" ^>^> "%%LOG_DIR%%\launcher.log" 2^>^&1
...
echo   %PATCH_RUNTIME%\run_patched_app.py
echo.
...
:dl
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%RAW%/%~1' -OutFile '%~2' -UseBasicParsing; exit 0 } catch { Write-Host $_.Exception.Message; exit 1 }"
if errorlevel 1 (
  echo   [FAIL] %~1
  set "FAILED=1"
) else (
