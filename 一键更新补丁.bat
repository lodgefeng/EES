...
if not exist "%PATCH_RUNTIME%\run_patched_app.py" (
  echo ERROR: run_patched_app.py still missing.
  pause
  exit /b 1
)
...
(
  ...
  echo if not exist "%%PATCH_RUNTIME%%\run_patched_app.py" (
  echo   echo ERROR: Missing %%PATCH_RUNTIME%%\run_patched_app.py
  echo   pause
  echo   exit /b 1
  echo ^)
...
echo   %PATCH_APP%\home_menu_patch.py
echo.
echo Start app with:
echo   %LAUNCHER%
echo.
